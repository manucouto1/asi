...
const VirtualClassroomsList = () => {
  ...
  useEffect(() => {
    ...
    async function fetchMyAPI() {
      ...
      const response =
        role === "secretario"
          ? await getGroups() : role === "profesor" ? await getTeacherGroups()
          : await getMyGroups();
  ...
  return (
    <div style={{ padding: "2%" }}>
      ...
      <p>
        {virtualClassrooms && virtualClassrooms !== undefined && virtualClassrooms.length === 0 && (
          <p>No hay disponible ningún aula virtual</p>
        )}
        {virtualClassrooms && virtualClassrooms !== undefined &&
